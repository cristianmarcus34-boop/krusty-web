...
// Constantes de Configuración Inmutable (Clean Code / Maintainability)
...
export default function KrustyLegalPage() {
  ...
  // Audio Context nativo para evitar dependencias pesadas y fugas de memoria
  const audioContextRef = useRef<AudioContext | null>(null);

  // Generamos el Hash del alma una sola vez para evitar mutaciones en re-renders
  const soulHash = useMemo(() => Math.random().toString(36).substring(2, 15).toUpperCase(), []);

  // Sintetizador de sonido "Risa de Krusty/Burlona" por Web Audio API
  const playKrustyLaughEffect = () => {
    if (audioMuted) return;
    try {
      // Solución deno-lint(no-window): Reemplazado 'window' por 'globalThis'
  ...
  return (
    <main className="min-h-screen bg-[#FAD02C] p-4 md:p-8 flex flex-col items-center justify-center font-sans antialiased selection:bg-[#E21B22] selection:text-white relative">
      ...
      {/* Botón superior izquierdo: Volver al menú principal */}
      <Link 
        href="/"
      ...
      {/* Solución deno-lint(jsx-button-has-type): Se añade type="button" */}
      <button 
        type="button"
        ...
        <AnimatePresence mode="wait">
          {!isSigned ? (
            ...
            >
              ...
              <div 
                className="space-y-6 max-h-[380px] overflow-y-auto pr-2 text-sm leading-relaxed border-2 border-black p-4 bg-white rounded-lg scrollbar-thin scrollbar-thumb-black scrollbar-track-transparent"
              ...
              <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t-2 border-black pt-6">
                ...
                <motion.button
                  type="button"
                  whileHover={accepted ? { scale: 1.03 } : {}}
                  whileTap={accepted ? { scale: 0.98 } : {}}
                  onClick={handleSignContract}
                  disabled={!accepted || isPending}
                  className={`w-full sm:w-auto px-8 py-4 font-black uppercase tracking-wider text-white border-4 border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all ${
                    accepted && !isPending
                      ? 'bg-[#E21B22] hover:bg-black cursor-pointer' 
                      : 'bg-gray-400 cursor-not-allowed opacity-50'
                  }`}
                >
                  {isPending ? 'PROCESANDO ALMA...' : 'FIRMAR CONTRATO ✍️'}
                </motion.button>
              </div>
            </motion.div>
          ) : (
            ...
            >
              ...
              {/* Botón secundario para volver tras la confirmación exitosa */}
              <Link 
                href="/"
                className="w-full bg-[#FAD02C] text-black font-black uppercase tracking-widest py-3 border-2 border-white rounded shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all text-xs"
              >
                Volver a la tienda
              </Link>
            </motion.div>
