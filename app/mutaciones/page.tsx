...
export default function MutacionesPage() {
            ...
            try {
                ...
                if (tipo === 'mutacion') {
                    ...
                    [0, 0.15].forEach((delay, i) => {
                        ...
                        const gain = ctx.createGain();
                        osc.type = 'sine';
                        osc.frequency.setValueAtTime(523 + i * 200, now + delay);
                    ...
                    [0, 0.2, 0.4].forEach((delay, i) => {
                        ...
                        const gain = ctx.createGain();
                        osc.type = 'square';
                        osc.frequency.setValueAtTime(440 + i * 150, now + delay);
    ...
    useEffect(() => {
        ...
        const draw = (): void => {
            ...
            for (let index = 0; index < particles.length; index++) {
                ...
                const opacity = Math.min(p.life / 40, 0.25);
                ctx.beginPath();
    ...
    // ============================================
    // FUNCIÓN SIMULAR MUTACIÓN CON TIMEOUT CORREGIDO
    // ============================================
    ...
    const simularMutacion = useCallback((): void => {
        ...
        // Crear el timeout y guardarlo en el ref
        timeoutRef.current = setTimeout(() => {
            setMutacionObtenida(mutacionElegida!);

            if (cliente) {
                const nuevasMutaciones = [...cliente.mutaciones, mutacionElegida!];
                const nuevaExperiencia = cliente.experiencia + Math.floor(Math.random() * 80) + 30;
            ...
            <main className="min-h-screen bg-gradient-to-br from-[#FAD02C] via-[#FFCA28] to-[#FFB300] p-4 md:p-8 flex flex-col items-center justify-center relative overflow-hidden">
                ...
                <div className="relative z-10 text-center max-w-2xl">
                    ...
                    <motion.button
                        ...
                        className="bg-black text-white font-black uppercase tracking-wider px-8 py-4 rounded-xl border-4 border-white transition-all text-base hover:bg-gray-900 active:scale-95"
                    >
                        🧬 Comenzar Mutación
                    </motion.button>
    ...
    return (
        <main className={`min-h-screen transition-colors duration-500 ${modoOscuro ? 'bg-gray-900 text-white' : 'bg-gradient-to-br from-[#FAD02C] via-[#FFCA28] to-[#FFB300] text-black'
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                    <Link
                        href="/"
                        className={`${modoOscuro ? 'bg-white text-black' : 'bg-black text-white'
                            } px-4 py-2 rounded-lg font-black text-sm uppercase tracking-wider hover:scale-105 transition-all shadow-md`}
                    >
                        ⬅ Volver
                    </Link>
                ...
                <motion.div
                    initial={{ opacity: 0, y: -20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className={`${modoOscuro ? 'bg-gray-800/80 border-gray-700' : 'bg-white/80 border-black'
                        } backdrop-blur-sm border-2 rounded-xl p-6 mb-8 shadow-xl`}
                >
                    <div className="flex flex-wrap items-center gap-6">
                        ...
                        <div className="flex flex-wrap gap-4 items-center">
                            <div className={`${modoOscuro ? 'bg-gray-700' : 'bg-black/10'
                    ...
                    <div className="mt-4">
                        ...
                        <div className={`w-full h-3 ${modoOscuro ? 'bg-gray-700' : 'bg-black/20'
                            } rounded-full overflow-hidden border-2 ${modoOscuro ? 'border-gray-600' : 'border-black'
                            }`}>
                            <motion.div
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className={`${modoOscuro ? 'bg-gray-800/80 border-gray-700' : 'bg-white/80 border-black'
                            } backdrop-blur-sm border-2 rounded-xl p-6`}>
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-xl font-black uppercase tracking-tighter">
                                    🧬 Tus Mutaciones
                                </h3>
                                <span className={`text-xs font-bold ${modoOscuro ? 'text-gray-400' : 'text-gray-600'
                                    }`}>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2">
                                    {cliente.mutaciones.map((mut, index) => (
                                        <motion.div
                                            ...
                                            className={`${modoOscuro ? 'bg-gray-700/50' : 'bg-black/5'
                        ...
                        <motion.button
                            ...
                            className={`w-full py-4 font-black uppercase tracking-wider text-base rounded-xl border-4 transition-all ${estaMutando
                    ...
                    {mutacionObtenida && (
                        ...
                        >
                            <motion.div
                                ...
                                className={`max-w-lg w-full ${modoOscuro ? 'bg-gray-800' : 'bg-white'
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-wrap gap-2 justify-center mb-6">
                                            {mutacionObtenida.efectos.map((efecto, i) => (
                                                <span key={i} className={`${modoOscuro ? 'bg-gray-700' : 'bg-black/10'
                                        ...
                                        <button
                                            ...
                                            onClick={() => setMutacionObtenida(null)}
                                            className={`${modoOscuro ? 'bg-white text-black' : 'bg-black text-white'
                                                } px-8 py-3 rounded-lg font-black uppercase tracking-wider text-sm border-2 transition-all hover:opacity-80`}
                                        >
                                            🚀 ¡Increíble!
                                        </button>
                                    </motion.div>
                    ...
                    {mutacionSeleccionada && (
                        ...
                        >
                            <motion.div
                                ...
                                animate={{ scale: 1, y: 0 }}
                                exit={{ scale: 0.8, y: 20 }}
                                className={`max-w-md w-full ${modoOscuro ? 'bg-gray-800' : 'bg-white'
                            ...
                            >
                                <div className="text-center">
                                    ...
                                    <div className="flex justify-center gap-2 mb-4">
                                        ...
                                        <span className={`text-xs font-black uppercase px-3 py-1 rounded-full ${modoOscuro ? 'bg-gray-700' : 'bg-black/10'
                                            }`}>
                                    ...
                                    <div className="text-left mb-6">
                                        ...
                                        <ul className="space-y-1">
                                            {mutacionSeleccionada.efectos.map((efecto, i) => (
                                                <li key={i} className={`text-sm flex items-center gap-2 ${modoOscuro ? 'text-gray-300' : 'text-gray-600'
                                                    }`}>
                                                    <span className="text-green-500">✦</span>
                                                    {efecto}
                                                </li>
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setMutacionSeleccionada(null)}
                                        className={`w-full py-3 rounded-lg font-black uppercase tracking-wider text-sm transition-all ${modoOscuro
