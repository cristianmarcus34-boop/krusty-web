...
export default function SeguimientoPedido() {
    ...
    const id = params?.id as string; // Forzamos a string para evitar líos
    const [pedido, setPedido] = useState<any>(null);
    const [loading, setLoading] = useState(true);
    
    // Usamos una referencia para saber si ya cargamos el pedido inicial
    ...
    useEffect(() => {
        ...
        // 2. Suscripción en Tiempo Real (Solo si tenemos ID)
        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    console.log("🔔 Estado actualizado:", payload.new.estado);
                    // Solo actualizamos si el estado realmente cambió para evitar re-renders innecesarios
                    setPedido(payload.new);
                }
            )
            .subscribe();

        // 3. LIMPIEZA: Crucial para evitar el bucle de conexiones
        return () => {
            console.log("🧹 Limpiando suscripción de pedido:", id);
            supabase.removeChannel(channel);
        };
    }, [id]); // Solo depende de id

    // ... resto de tus funciones (handleVerTicket, estados, etc.) igual ...

    const handleVerTicket = () => {
        localStorage.setItem('ultimo_pedido_id', id);
        router.push('/gracias');
    };
    ...
    if (!pedido) return (
        <div className="min-h-screen bg-white flex flex-col items-center justify-center font-black italic uppercase p-4 text-center">
            ...
            <p className="mb-6 text-2xl text-black">Pedido no encontrado</p>
            <button
                onClick={() => router.push('/')}
    ...
    return (
        <div className="min-h-screen bg-stone-100 p-4 font-sans text-black pb-20">
            {/* ... Tu JSX de la barra de progreso y tarjeta de estado ... */}
            <div className="max-w-lg mx-auto pt-6">

                {/* NAVEGACIÓN */}
                <div className="flex justify-between items-center mb-6 gap-2">
                    <button onClick={() => router.push('/')} className="flex-1 flex items-center justify-center gap-2 font-black italic uppercase text-[10px] bg-white border-[3px] border-black px-4 py-2 rounded-xl shadow-[4px_4px_0px_black] active:translate-y-1 active:shadow-none transition-all">
                        ← Menú
                    </button>
                    <button onClick={handleVerTicket} className="flex-1 flex items-center justify-center gap-2 font-black italic uppercase text-[10px] bg-[#FFCA28] border-[3px] border-black px-4 py-2 rounded-xl shadow-[4px_4px_0px_black] active:translate-y-1 active:shadow-none transition-all">
                        Ver Ticket 🎫
                    </button>
                </div>
                ...
                <div className="bg-[#D32F2F] border-[6px] border-black p-6 rounded-[2.5rem] shadow-[8px_8px_0px_0px_black] text-white text-center mb-8">
                    <h1 className="text-3xl font-black italic uppercase tracking-tighter drop-shadow-[2px_2px_0px_black]">Tu Pedido</h1>
                </div>

                {/* INDICADOR EN VIVO */}
                <div className="flex justify-between items-end mb-4 px-2">
                    <p className="font-black text-[10px] uppercase bg-black text-white px-3 py-1 rounded-full">
                        Orden: #{String(id).slice(-6)}
                    </p>
                ...
                <div className="bg-white border-[6px] border-black p-8 rounded-[3rem] shadow-[10px_10px_0px_0px_black] text-center mb-8">
                    <div className="mb-4 text-7xl inline-block animate-bounce">
